          ...
          <div>
            ...
            <div class="relative aspect-[3/4] bg-stone-100 overflow-hidden rounded mb-2 select-none">
              ...
              <div class="absolute top-1.5 sm:top-2 right-1.5 sm:right-2 flex flex-col gap-1.5 z-10" onclick="event.stopPropagation()">
                <div class="heart-container w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/95 hover:bg-white border border-stone-200 shadow-2xs transition-all hover:scale-110 active:scale-90 flex items-center justify-center cursor-pointer" title="Save to Wishlist" data-wishlist-id="<?= (int)$p['id'] ?>" onclick="handleHeartClick(this, {id:<?= (int)$p['id'] ?>, title:<?= htmlspecialchars(json_encode($p['title'] ?? ''), ENT_QUOTES, 'UTF-8') ?>, price:<?= (float)$b_price ?>, image:<?= htmlspecialchars(json_encode($img1 ?? ''), ENT_QUOTES, 'UTF-8') ?>}, event)">

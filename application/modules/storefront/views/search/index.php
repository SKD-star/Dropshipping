...
<main class="min-h-screen bg-[#FAF8F5] text-stone-900 pt-20 sm:pt-24 pb-28">
  <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-10">
    ...
    <!-- Breadcrumb & Header -->
    <div class="mb-6 sm:mb-8 border-b border-stone-200 pb-6 flex flex-col md:flex-row justify-between items-start md:items-end gap-4">
      <div>
        <nav class="text-xs font-mono text-stone-500 flex items-center gap-2 uppercase tracking-wider mb-2">
          ...
          <span>/</span>
          <span class="text-[#a16207] font-semibold">Search Archive</span>
        </nav>
      ...
      <div class="flex items-center gap-3">
        <a href="<?= base_url('shop') ?>" class="px-5 py-2.5 bg-stone-950 text-[#e9c176] text-xs font-mono font-bold uppercase tracking-wider rounded-xl hover:bg-stone-800 transition-all shadow-md">
          Explore Boutique →
        </a>
      </div>
    </div>
    ...
    <div class="bg-white rounded-3xl p-4 sm:p-6 border border-stone-200 shadow-sm mb-8 sm:mb-10">
      <form action="<?= base_url('search') ?>" method="GET" class="flex flex-col sm:flex-row items-center gap-3">
        <div class="relative flex-1 w-full">
          <span class="material-symbols-outlined absolute left-3.5 top-1/2 -translate-y-1/2 text-[#a16207]">search</span>
          <input type="text" name="q" value="<?= htmlspecialchars($query ?? '') ?>" placeholder="Search cashmere coats, mulberry silk, selvedge denim..." class="w-full pl-11 pr-4 py-3 bg-stone-50 border border-stone-200 focus:border-[#a16207] rounded-2xl text-sm sm:text-base font-serif text-stone-900 outline-none transition-all placeholder:text-stone-400">
        </div>
        <button type="submit" class="w-full sm:w-auto px-7 py-3 bg-stone-950 hover:bg-stone-800 text-white font-mono font-bold text-xs uppercase tracking-widest rounded-2xl transition-all shadow-md cursor-pointer flex items-center justify-center gap-2 flex-shrink-0">
          <span>Search</span>
      ...
      <div class="mt-4 pt-3 border-t border-stone-100 flex items-center gap-2 overflow-x-auto custom-scrollbar pb-1">
        <span class="text-[10px] font-mono uppercase tracking-widest text-stone-400 font-bold flex-shrink-0">Trending:</span>
        <a href="<?= base_url('search?q=Silk') ?>" class="px-3 py-1 bg-amber-50 hover:bg-amber-100 text-[#a16207] border border-amber-200 text-xs font-mono rounded-full transition-all flex-shrink-0">✨ Mulberry Silk</a>
        <a href="<?= base_url('search?q=Cashmere') ?>" class="px-3 py-1 bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-200 text-xs font-mono rounded-full transition-all flex-shrink-0">🧥 Cashmere Coat</a>
        <a href="<?= base_url('search?q=Denim') ?>" class="px-3 py-1 bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-200 text-xs font-mono rounded-full transition-all flex-shrink-0">👖 Selvedge Denim</a>
        <a href="<?= base_url('search?q=Boot') ?>" class="px-3 py-1 bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-200 text-xs font-mono rounded-full transition-all flex-shrink-0">👞 Chelsea Boots</a>
        <a href="<?= base_url('search?q=Trench') ?>" class="px-3 py-1 bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-200 text-xs font-mono rounded-full transition-all flex-shrink-0">🧥 Atelier Trench</a>
        <a href="<?= base_url('search?q=Blazer') ?>" class="px-3 py-1 bg-stone-100 hover:bg-stone-200 text-stone-800 border border-stone-200 text-xs font-mono rounded-full transition-all flex-shrink-0">👔 Wool Blazer</a>
      </div>
    </div>
          ...
          <div>
            <div class="relative aspect-[3/4] bg-stone-100 rounded-xl overflow-hidden mb-3">
              ...
              <div class="absolute top-2.5 right-2.5 z-10" onclick="event.stopPropagation()">
                <div class="heart-container w-7 h-7 sm:w-8 sm:h-8 rounded-full bg-white/95 hover:bg-white border border-stone-200 shadow-xs transition-all hover:scale-110 active:scale-90 flex items-center justify-center cursor-pointer" title="Save to Wardrobe" data-wishlist-id="<?= (int)$p['id'] ?>" onclick="handleHeartClick(this, {id:<?= (int)$p['id'] ?>, title:<?= htmlspecialchars(json_encode($p['title'] ?? ''), ENT_QUOTES, 'UTF-8') ?>, price:<?= (float)$price ?>, image:<?= htmlspecialchars(json_encode($img ?? ''), ENT_QUOTES, 'UTF-8') ?>}, event)">
                  <input type="checkbox" class="checkbox" data-wishlist-id="<?= (int)$p['id'] ?>" tabindex="-1" style="pointer-events: none;">
